...
public class GenerateModel2Local {

	public static void main(String[] args) {

		//Number of Events per Year/Devices/Country
		//ano, device_info_manufacturer, device_info_model, user_country, sum(*), group by (ano, device_info_manufacturer, device_info_model, user_country)
		...
		final Schema SCHEMA = Schema.builder()
				...
				.addStringField("user_country").build();
					...
					public void processElement(ProcessContext c) {
						Row appRow = Row
								...
								.addValues(c.element().getYear(), c.element().getDevice_info_manufacturer(), 
										   c.element().getDevice_info_model(), c.element().getUser_country())
								.build();
						c.output(appRow);
					}
				})).setRowSchema(SCHEMA);
		...
		PCollection<Row> rowSqlTransform = rowCollection.apply("Transform SQL", 
				SqlTransform.query(
						"SELECT ano, device_info_manufacturer, device_info_model, user_country, count(1) qtde " +
						"  FROM PCOLLECTION " +
						" GROUP BY ano, device_info_manufacturer, device_info_model, user_country")
				);
